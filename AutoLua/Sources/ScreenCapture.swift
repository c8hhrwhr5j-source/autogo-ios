...
/// 屏幕截图 —— 流式后台捕获 + 多点找色
/// 核心机制参考 AutoLua 的 RawFrameSetCallback + RawFrameStart
final class ScreenCapture {
    ...
    func startStreaming(fps: Int = 20) {
        ...
        // 4. 启动定时器，持续捕获 (AutoLua: RawFrameStart → 回调循环)
    ...
    // MARK: - 帧获取 (AutoLua: CaptureScreen → 从回调缓存取)
    ...
    // MARK: - 多点找色 (AutoLua: FindMultiColors)
    ...
    // MARK: - 字符串格式多点找色 (AutoLua: FindMultiColors("0|0|FF0000,10|5|00FF00"))

    /// 字符串格式多点找色 (与 AutoLua 协议兼容)
    ///
    /// - Parameters:
