...
/// 统一管理所有核心模块的启动、停止、状态
final class AutoLuaCore {

    static let shared = AutoLuaCore()
    ...
    func start() {
        ...
        print("[AutoLua] ✅ 全部服务已启动")
        print("[AutoLua] 🌐 HTTP 服务器: http://127.0.0.1:8989")
        print("[AutoLua] 📱 屏幕: \(screenCapture.cachedWidth)x\(screenCapture.cachedHeight)")
    }
    ...
    private func handleSystemInfo(_ request: HttpServer.Request) -> HttpServer.Response {
        ...
        return .json([
            ...
            "scale": screen.scale,
            "identifier": Bundle.main.bundleIdentifier ?? "com.autolua.ios"
        ])
    }
